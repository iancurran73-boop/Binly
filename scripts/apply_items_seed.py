#!/usr/bin/env python3
"""Apply seed_items.sql via Supabase Management API (using anon-PostgREST RPC won't work for inserts with cross-join,
   so we just execute the SQL via psql equivalent — but we don't have direct DB conn. So instead, we'll insert items
   programmatically via the REST API by reading the council list and the items definition)."""
import json
...
# Items defined inline to mirror seed_items.sql (33 items)
ITEMS = [
    # (item_name, bin_type, category, notes, tip, fun_fact)
...
# Fetch all council IDs from Supabase
